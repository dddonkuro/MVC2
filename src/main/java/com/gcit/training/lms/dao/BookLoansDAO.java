...
public class BookLoansDAO extends AbstractDAO implements ResultSetExtractor<List<BookLoans>>
{
	...
	{
		template.update("insert into tbl_book_loans (bookId, branchId, cardNo, dateOut, dueDate) values (?,?,?,curdate(),DATE_ADD(curdate(),INTERVAL 5 DAY))", new Object[] { a.getBook().getBookId(), a.getBranch().getBranchId(),
				a.getCard().getCardNo()});
	}
	...
	public void update(BookLoans a) throws SQLException
	{
		...
		template.update("update tbl_book_loans set dateIn = curDate() where bookId = ? AND branchId = ? AND cardNo = ?", new Object[] { a.getDateIn(), a.getBook().getBookId(),
				a.getBranch().getBranchId(), a.getCard().getCardNo()});
	}
	public Integer bookLoanCount()
	{
	...
	public List<BookLoans> readByBranchId(int branchId,int pageSize) throws SQLException
	{
